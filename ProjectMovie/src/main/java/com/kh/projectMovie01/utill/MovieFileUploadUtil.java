...
public class MovieFileUploadUtil {
	
	@Resource
	private static String uploadPath;
	
	// 사진등록
	public static String uploadFile(String MovieUploadPath, String originalName, byte[] fileData) throws Exception {
		UUID uuid = UUID.randomUUID();
		String datePath = calcPath(MovieUploadPath);
		String dirPath = datePath + File.separator + uuid + "-" + originalName;
		String filePath = MovieUploadPath + File.separator + dirPath;
	...
	// 
	private static String calcPath(String MovieUploadPath) {
		...
		String datePath = MovieUploadPath + File.separator + dateString;
	...
	// 섬네일등록
	public static String uploadPreviewFile(String MovieUploadPath, String originalName, byte[] fileData) throws Exception {
		UUID uuid = UUID.randomUUID();
		String datePath = calcPath(MovieUploadPath);
		String dirPath = datePath + File.separator + uuid + "-" + originalName;
		String filePath = MovieUploadPath + File.separator + dirPath;
	...
	public static String deleteFile(String fileName) throws Exception {
		String MovieUploadPath = uploadPath + "/upload";
		String filePath = MovieUploadPath + File.separator + fileName;
		
		String file2Front = fileName.substring(0, fileName.lastIndexOf("/") + 1);
		String file2Rear = fileName.substring(fileName.lastIndexOf("/") + 1);
		String smFilePath = MovieUploadPath + File.separator + file2Front + "sm_" + file2Rear;
		
		File file = new File(filePath);
		file.delete();
		
		boolean result = MovieFileUploadUtil.isImage(fileName);
		if (result == true) {
